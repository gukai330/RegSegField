...
@dataclass
class SAMGaussianSplattingModelConfig(ModelConfig):
    """Gaussian Splatting Model Config"""

    _target: Type = field(default_factory=lambda: SAMGaussianSplattingModel)
...
class SAMGaussianSplattingModel(Model):
    ...
    config: SAMGaussianSplattingModelConfig
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        rgb, output_vis, output_alpha = rasterize_gaussians(
            self.xys,
            depths,
            self.radii,
            conics,
        ...
        if not self.training:
            depth_im,*_ = rasterize_gaussians(  # type: ignore
                self.xys,
                depths,
                self.radii,
                conics,
        ...
        return {"rgb": rgb, "depth": depth_im, "vis": output_vis}  # type: ignore
    ...
    def get_vis_loss(self, cameras:Cameras, out_image,out_vis, source_mask, target_masks):
        ...
        source_mask_gaussians = source_mask[self.xys]


        # assert cameras.shape[0] == 1, "Only one camera at a time"

        if self.training:
            self.camera_optimizer.apply_to_camera(cameras)
        ...
        with torch.no_grad():
            ...
            for i in range(len(target_masks)):
                ...
                projmat = projection_matrix(znear=1, zfar=100, fovx=fovx, fovy=fovy, device=self.device)
                ...
                xys_target, depths, self.radii, conics, num_tiles_hit, cov3d = ProjectGaussians.apply(
                    means_crop,
                    torch.exp(scales_crop),
                    1,
                    quats_crop / quats_crop.norm(dim=-1, keepdim=True),
                ...
                target_mask_gaussians = target_mask[xys_target]
                
                matches = torch.abs(target_mask_gaussians - source_mask_gaussians) < 1e-3
        

                visibility += matches
        
        # to loss
        penality_size = self.mask_count_thresh - visibility
