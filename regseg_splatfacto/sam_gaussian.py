...
@dataclass
class SAMSplatfactoModelConfig(ModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: SAMSplatfactoModel)
...
class SAMSplatfactoModel(SplatfactoModel):
    ...
    config: SAMSplatfactoModelConfig
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if self.crop_box is not None and not self.training:
            ...
            if crop_ids.sum() == 0:
                return {"rgb": background.repeat(int(camera.height.item()), int(camera.width.item()), 1),
                        "depth": torch.ones((int(camera.height.item()), int(camera.width.item()), 1), device=self.device, dtype=torch.float32)}
        else:
            crop_ids = None
        ...
        if (self.radii).sum() == 0:
            return {"rgb": background.repeat(int(camera.height.item()), int(camera.width.item()), 1),
                    "depth": torch.ones((int(camera.height.item()), int(camera.width.item()),1), device=self.device)}
        ...
        return {"rgb": rgb, "depth": depth_im, "visibility": visibility}  # type: ignore
    ...
    def get_vis_loss(self, cameras:Cameras, out_image,out_vis, source_mask, target_masks, step):
        
        d = self._get_downscale_factor()

        # randomly select 6 views from the target_masks

        random_idx = torch.randperm(len(target_masks))[:12]
        ...
        with torch.no_grad():
            ...
            for i in random_idx:
                camera = cameras[i].to(self.device)
                ...
                if reg_mode == "frustum":
                    ...
                    if len(target_mask.shape) == 4:
                        ...
                        matches = torch.sum(matches, dim = 1)
    ...
    def get_metrics_dict(self, outputs, batch) -> Dict[str, torch.Tensor]:
        ...
        if self.config.use_depth:
            ...
            midas_depth = batch["depth_image"].to(self.device).view(-1, 1)
            rendered_depth = outputs["depth"].view(-1, 1)
            ...
            metrics_dict["pearson_depth_loss"] = depth_loss
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        if self.config.use_depth and "pearson_depth_loss" in metrics_dict:
            ...
            depth_loss = metrics_dict["pearson_depth_loss"] * 0.05
