...
class SAMDataset(InputDataset):
    ...
    def __init__(self, dataparser_outputs: DataparserOutputs, scale_factor: float = 1.0):
        ...
        self.maskloader = dataparser_outputs.maskloader

        self.load_binary_masks = False
        
        self.binary_masks = None

        if dataparser_outputs.mask_filenames is not None:
            if "masks_manual" in str(dataparser_outputs.mask_filenames[0]):

                self.load_binary_masks = True

            elif dataparser_outputs.binary_masks is not None:

                self.load_binary_masks = True
                self.binary_masks = dataparser_outputs.binary_masks
        # self.maskloader = None

    # def load_matched_masks(self, ):
    #     if self._dataparser_outputs.mask_filenames is None:
    #         self.maskloader = None
    #         return
    #     config = MaskMatcherLoaderConfig(
    #         data_dir = Path(f"/mnt/e/data/360_v2/counter"),
    #         ckpt_path=Path("counter_12.pth"),
    #         mask_filename = self._dataparser_outputs.mask_filenames,
    #         downscale_factor=8,
    #         colmap_path=Path("sparse/0"),
    #     )
    #     self.maskloader = config.setup()
    ...
    def get_metadata(self, data: Dict) -> Dict:
        ...
        # if "camera_closeness_matrix" in self._dataparser_outputs.metadata:
        #     metadata["camera_closeness_indices"] = \
        #         self._dataparser_outputs.metadata["camera_closeness_matrix"][data["image_idx"]]


        if self.depth_filenames is None:
            if self.depths is not None:
                metadata["depth_image"] = self.depths[data["image_idx"]]
            return metadata
    ...
    def get_masks(self, image_idx: int):
        ...
        if self.maskloader is not None:
            masks = self.maskloader[image_idx]
            
        elif self._dataparser_outputs.matched_masks is not None:            

            masks = self._dataparser_outputs.matched_masks[image_idx]
            # to tensor
            masks = torch.from_numpy(masks).float()
        
            if self.load_binary_masks and self.binary_masks is None:
                # manual option

                # visualize only mask 5
                foreground = masks > 2
                masks[~foreground] = -1
    ...
    def get_mask(self, image_idx: int):
        ...
        if self.binary_masks is not None:
            mask = torch.from_numpy(self.binary_masks[image_idx])
            ...
            # hardcoded toy data
            mask = torch.from_numpy(masks>2)

            # do a erosion
            # mask = F.avg_pool2d(mask.unsqueeze(0).unsqueeze(0).float(), 3, stride=1, padding=1, count_include_pad=False).squeeze(0).squeeze(0)
