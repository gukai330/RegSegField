...
rasterize_forward_tensor(
    ...
    torch::Tensor out_vis = torch::zeros(
        {num_points, 1}, xys.options().dtype(torch::kFloat32)
    );
    ...
    rasterize_forward<<<tile_bounds_dim3, block_dim3>>>(
        ...
        out_vis.contiguous().data_ptr<float>(),
        (float3 *)out_img.contiguous().data_ptr<float>(),
        *(float3 *)background.contiguous().data_ptr<float>()
    );

    return std::make_tuple(out_img, out_vis, final_Ts, final_idx);
}
...
std::
    ...
    nd_rasterize_backward_tensor(
        ...
        const torch::Tensor &v_output_vis, //dL_dout_vis
...
std::
    ...
    rasterize_backward_kernel<<<tile_bounds, block>>>(
        ...
        *(float3 *)background.contiguous().data_ptr<float>(),
        ...
        v_output_vis.contiguous().data_ptr<float>(),
        v_output_alpha.contiguous().data_ptr<float>(),
        (float2 *)v_xy.contiguous().data_ptr<float>(),
        (float3 *)v_conic.contiguous().data_ptr<float>(),
