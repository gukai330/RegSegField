...
__global__ void rasterize_backward_kernel(
    ...
    const float* __restrict__ v_output_vis,
    const float* __restrict__ v_output_alpha,
    float2* __restrict__ v_xy,
    float3* __restrict__ v_conic,
    ...
    // the accumulated contributions without color
    // float vis_buffer = 0.f;
    ...
    __shared__ float v_out_vis_batch[BLOCK_SIZE];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range.x) {
            ...
            v_out_vis_batch[tr] = v_output_vis[g_id];
        }
