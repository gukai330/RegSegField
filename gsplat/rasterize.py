...
class _RasterizeGaussians(Function):
    ...
    def forward(
        ...
        if len(res) == 4:
            out_img, out_vis, final_Ts, final_idx = res
        
        else:
            out_img, final_Ts, final_idx = res
        ...
        if return_alpha:
            ...
            return out_img, out_vis, out_alpha
        else:
            return out_img

    @staticmethod
    def backward(ctx, v_out_img, v_out_vis= None, v_out_alpha=None):
        ...
        if v_out_vis is None:    
            v_out_vis = torch.zeros_like(opacity)
